            ...
            try
            {
                ...
                using var stream = new MemoryStream();
                {
                    using var writer = new IndentedTextWriter(new StreamWriter(stream,  Encoding.UTF8), " ");
                    {
                        ...
                        writer.WriteLineEx("include!(concat!(env!(\"OUT_DIR\"), concat!(\"/\", \"_\", \".rs\")));");
                        ...
                        writer.WriteLineEx($"let mut file = std::fs::File::open(r\"../../GameDesign/TBL/Global/{filename}.bytes\").unwrap();");
