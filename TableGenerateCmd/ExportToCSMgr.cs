...
namespace TableGenerate
{
    public class ExportToCSMgr : ExportBase
    {
                ...
                var _writer = new IndentedTextWriter(new StreamWriter(stream, new System.Text.ASCIIEncoding()), " ");
                {
                    ...
                    if(_async == "unity3d")
                    {
                        ...
                        foreach (string sheetName in sheets)
                        {
                            ...
                            _writer.WriteLineEx($"public System.Collections.Generic.List<{NameSpace}.{filename}.{trimSheetName}> {trimSheetName} = new System.Collections.Generic.List<{NameSpace}.{filename}.{trimSheetName}>();");
                        }
                        _writer.WriteLineEx("}");
                        ...
                        {
                            string trimSheetName = sheetName.Trim().Replace(" ", "_");
                            _writer.WriteLineEx($"myScript.{trimSheetName} = {NameSpace}.{filename}.{trimSheetName}._array;");
                        }
                        _writer.WriteLineEx("}");
                    ...
                    _writer.WriteLineEx("{");
                    _writer.WriteLineEx("language = language.Trim().ToLower();");
                    ...
                    _writer.WriteLineEx("/*");
                    _writer.WriteLineEx("public void GetMapAndArray(System.Collections.Generic.Dictionary<string,object> container)");
                    _writer.WriteLineEx("{");
                    foreach (string sheetName in sheets)
                    {
                        string trimSheetName = sheetName.Trim().Replace(" ", "_");
                        ...
                        _writer.WriteLineEx($"container.Remove( \"{filename}.{trimSheetName}._map\");");
                        _writer.WriteLineEx($"container.Remove( \"{filename}.{trimSheetName}._array\");");
                        _writer.WriteLineEx($"container.Add( \"{filename}.{trimSheetName}._map\", new System.Collections.Generic.Dictionary<{type},{trimSheetName}>({trimSheetName}._map{(string.IsNullOrEmpty(equality_type)?string.Empty:$",{equality_type}")}));");
                        _writer.WriteLineEx($"container.Add( \"{filename}.{trimSheetName}._array\",new System.Collections.Generic.List<{trimSheetName}>({trimSheetName}._array));");
                    }
                    _writer.WriteLineEx("}");
                    _writer.WriteLineEx("*/");
        ...
        private void GetDataTableFunction(IndentedTextWriter _writer, string filename, string sheetName, List<Column> columns)
        {
            ...
            _writer.WriteLineEx("foreach(var item in _array )");
            _writer.WriteLineEx("{");
        ...
        private void SetDataTableFunction(IndentedTextWriter _writer, string filename, string sheetName, List<Column> columns)
        {
            ...
            _writer.WriteLineEx("{");
            _writer.WriteLineEx($"{sheetName}._map = new System.Collections.Generic.Dictionary<{key_type},{sheetName}>();");
            _writer.WriteLineEx($"{sheetName}._array = new System.Collections.Generic.List<{sheetName}>();");
            ...
            _writer.WriteLineEx(");");
            _writer.WriteLineEx($"{sheetName}._map.Add(table.{key_column.var_name}, table);");
            _writer.WriteLineEx($"{sheetName}._array.Add(table);");
            _writer.WriteLineEx("}");
